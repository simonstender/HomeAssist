				...
				if (data[i].lights == 'On') {
					for (var j = 0; j < Object.keys(this.state.data).length; j++) {
						if (this.state.data[j].name == data[i].room) {
							...
							this.updateRoom(this.state.data[j], "On", "green", "Hold")
						}
					}
				} else if (data[i].lights === "Off") {
					for (var j = 0; j < Object.keys(this.state.data).length; j++) {
						if (this.state.data[j].name == data[i].room) {
						...
						if (this.state.data[j].devices == this.state.numberOfDevices[j] ) {
							...
							this.updateRoom(this.state.data[j], "Off", "red", "Hold")
						}
					}
				}
			}
			...
			.then((data) => {
				for (var i = 0; i < Object.keys(data).length; i++) {
					if (data[i].room == item.name) {
						if (allLights == "On") {
							this.updateDevice(data[i], "On", "green")
						} else if (allLights == "Off") {
					 		this.updateDevice(data[i], "Off", "red");
						}
					}
				}
			})
